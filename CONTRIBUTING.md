...
- **Offline GPU-free batch repro:** `scripts/mock_comfy.py` stands up a fake ComfyUI so the app runs **real local batches with no GPU / no spend** (fire Generate batch… against it). `--fail-rate F` injects FAILED takes to exercise crash-recovery / Restart-take paths. Keep the real ComfyUI down so port 8188 is free; see "Offline GPU-free harness" in `CLAUDE.md`.
