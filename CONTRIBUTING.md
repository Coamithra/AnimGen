...
AnimGen is a native **PySide6 desktop app** (Python 3.12) — there is no build step, no dev server, and no browser preview. The gate before shipping is the **headless smoke suite** (`scripts/smoke_phase1-7.py`), not a typecheck/`npm test`. (`scripts/smoke_phase8.py` is an **opt-in** offline integration smoke that drives one real take through the mock ComfyUI `submit -> /history -> claim` path end-to-end; it is NOT part of the always-run 1-7 gate.) (You can also drive the live GUI without full-PC control via the opt-in localhost control server — launch with `ANIMGEN_REMOTE=1` and use `scripts/remote_cli.py`; see `remote/` in `CLAUDE.md`. For local-backend / queue work, `scripts/mock_comfy.py` is the supported **offline GPU-free** harness — a fake ComfyUI the unmodified app drives real batches through with no GPU / no spend; see "Offline GPU-free harness" in `CLAUDE.md`.)
...
- **Offline GPU-free batch repro:** `scripts/mock_comfy.py` stands up a fake ComfyUI so the app runs **real local batches with no GPU / no spend** (fire Generate batch… against it). `--fail-rate F` injects *genuine* FAILED takes (server stays up) to exercise crash-recovery's not-a-crash discrimination + the FAILED-take path. Keep the real ComfyUI down so port 8188 is free; see "Offline GPU-free harness" in `CLAUDE.md`.
...
14. **Make the changes** — Edit files per the approved plan. Follow project conventions:
    ...
    - **Local backend MUST run with `--disable-dynamic-vram --disable-async-offload`** (both PCIe weight-streaming paths off; either trips the GPU watchdog / TDR crash on the 12GB card). AnimGen doesn't start ComfyUI, so `comfy_client.preflight()` refuses to submit a local job if *either* path is on. Don't weaken that gate (escape hatch: `ANIMGEN_ALLOW_DYNAMIC_VRAM=1`)
...
| **Backends — local (ComfyUI)** | `backends/comfy_client.py` + lifecycle/preflight. MUST run `--disable-dynamic-vram --disable-async-offload` (both PCIe weight-streaming paths; GPU watchdog/TDR crash); `preflight()` refuses if either is on. Probing a down port costs a full socket timeout on this machine — keep status polling off the GUI thread. A live take uses GPU — explicit go-ahead only. Verify offline GPU-free with `scripts/mock_comfy.py` (fake ComfyUI; `--fail-rate` injects genuine FAILED takes for crash-recovery's not-a-crash discrimination) |
| **Framing pipeline** | `pipeline/framing.py` — `normalize_keypose`, `canvas_size` (hosted: longest side 1254; local: ~410k-px budget snapped to /16), `render_keyposes` keys + places sprites at generation time. Placement `scale` is relative to the **original image** (whole frame keyed transparent, `keyed_sprite(crop_to_content=False)`), not the cutout. No baked keypose files. Eyeball across wide/tall aspects |
