...
| `pipeline/takes_io.py` | bin / restore (only files under the project's `.assets/`; external refs left in place). **`move_to_bin` is per-move atomic (M13):** it flips `deleted=True` and writes each successful file move's new path through to the take AS IT HAPPENS (not one batched `update_take` at the end), so a transient AV/indexer lock on a *later* move (the documented Windows failure mode) leaves the record consistent with disk — the already-moved video keeps its `.bin` path and the take is flagged deleted, instead of the video sitting in `.bin` while the record still points at the vanished old path and is never marked deleted. `restore_from_bin` restores such a partial bin cleanly (it moves back only files actually under `.bin`, skipping ones that never moved). The caller `TakesView.delete` wraps the per-take `move_to_bin` in `try/except OSError` so one take's failed move can't abort binning the rest of a multi-delete |
...
2. **Additive — copy in, never move external originals.** Importing a keyframe asset
   ...
   restores such a partial bin cleanly; `TakesView.delete` swallows a take's `OSError` so one
