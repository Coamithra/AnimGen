...
| `pipeline/gif_export.py` | take → **animated GIF** (pure / Qt-free, headless-testable): `encode_gif(frames, out, fps, *, loop=0, max_side=None)` writes an animated GIF via Pillow (`save_all`, per-frame `duration` from fps, `loop=0`, `disposal=2`); `take_to_gif(source, out, ...)` decodes a take's video with `extract.iter_frames` (PyAV → PIL) and delegates. Backs the take player's right-click *Save as GIF…* / *Copy GIF to clipboard* (rule #3) |
