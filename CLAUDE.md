...
| `store/project.py` `store/models.py` | file-based **Project** document (shots / takes / jobs) + dataclasses (`Shot`/`Take`/`Job`). Hybrid persistence: shot edits buffer (`dirty`, saved on `save()`); takes write through to `<assets>/takes.json`. Keyframe **assets** (`list_assets`/`import_asset`/`remove_asset` — image files flat in `.assets/`; per-asset **background metadata** in an `assets_meta.json` sidecar + transparent references under `.assets/.originals/` via `asset_meta`/`set_asset_meta`/`store_transparent_ref`/`transparent_ref`/`clear_asset_meta` — keyed by asset filename, read on demand, carried by `save_as` and excluded from `list_assets`; backs the Replace-background reuse path, rule #22) + a load-time migration that flattens old `keyposes/<hash>/` baked files (`_migrate_flatten_keyposes`: re-points the shot to an imported copy of its source, **persists the re-point write-through (`_write_project_file`) BEFORE deleting the keyposes sources** so a Discard at the next save-prompt can't strand a half-applied, source-deleted migration — and so the freshly-loaded project stays clean, no phantom `*`; a failed persist keeps the sources and falls back to `dirty=True`). Carries an optional `ui_state` window-layout blob (open tabs; UI-owned, written only when non-empty — see the Project model section). Shots carry a `starred` flag (`set_shot_starred`) that — unlike other shot edits — **writes through** to a `<assets>/shot_stars.json` sidecar (`_write_shot_stars_file`/`_load_shot_stars`), so a star persists instantly with the same timing as a take's star, without flushing buffered shot edits or marking the project dirty. The star is no longer serialized into the `.animproj`; a legacy `.animproj` still carrying `starred:true` migrates into the sidecar on load (sidecar authoritative thereafter) — and a sidecar that is **present but unreadable** falls through to that same migration path (re-materialized from the in-memory legacy flags), because the next ordinary Save strips `starred` from the `.animproj`, so leaving a corrupt sidecar in place would silently lose those stars for good (card #55). A Take records `created` (queued) **and `started`** (stamped at the GENERATING transition in `jobs.GenerationJob.run`), so the Queue tab's "done in X" shows render duration (`started`→`completed`) not queue wait — additive field, `None` for pre-existing takes (falls back to `created`). `purge_takes(ids)` is a **hard delete** — drops takes from `takes.json` entirely (no bin, no restore; distinct from `soft_delete_take`'s move-to-bin) and best-effort unlinks each take's **managed** media (files under `.assets`; an external ref is left in place, gotcha #2), one write for the batch; backs the Edit-menu *Remove cancelled takes*. RLock-guarded; atomic JSON writes |
| `backends/replicate_client.py` | hosted generation (refactor of Fighter's `run_replicate.py`). `get_input_schema` **inlines enum `$ref`s** (`_resolve_enums`/`_follow_enum`/`_deref`): Replicate stores a property's allowed values as a `$ref`/`allOf`/`anyOf`/`oneOf` into `components.schemas`, not inline, so the resolver pulls each referenced `enum` (+ `type`) onto the property before returning — without this the editor never sees live options and silently falls back to the authored lists. `run_prediction`/`generate` take an `on_submit(pred_id)` callback (mirrors comfy's) that fires right after the create POST so the take records `backend_job_id` mid-render and becomes cancellable; `cancel_prediction(pred_id)` POSTs `/predictions/{id}/cancel` (best-effort, stops spend). **`get_prediction(pred_id)`** GETs `/predictions/{id}` (idempotent → retried, **NO spend — observe only**) and **`download_output(prediction, out_path)`** fetches a *succeeded* prediction's video (idempotent GET, same retry-with-backoff as the render path) — the pair backs hosted orphan reconciliation on load (card L5): reconcile a take a prior session left GENERATING without re-running/re-charging it. **`api_request` also retries bare transport errors (`URLError`/`TimeoutError`/`socket.timeout` — a connection reset / DNS blip / read timeout), folding them into the same 5xx/429 backoff loop, but ONLY for idempotent GETs (`_is_idempotent`: no request body and no explicit non-GET method) — a non-idempotent create/upload POST is NOT network-retried (a 504 may have committed the create, so a retry double-spends, finding M2) and wraps on the first blip; a GET wraps in `ReplicateError` (`_network_error_message`) only after exhausting all 8 attempts. The result download (`_download_result`) has the same retry-with-backoff since it's an idempotent GET, and reads the whole body into memory before writing so a truncated read never leaves a partial file (card H4)** |
| `backends/comfy_client.py` | local ComfyUI generation (node-role mapping); also server lifecycle/status/preflight: `launch_server` (tracks the Popen in `_server_proc`), `stop_work(prompt_id=None)` (interrupt + drop **just our** pending prompt via `POST /queue {delete:[id]}`, falling back to `{clear:True}` only when no id is known — targeting avoids nuking foreign prompts and avoids the take sitting GENERATING for the full 1h timeout with no `/history` entry, card L8; `jobs.request_stop`/`stop_and_requeue` pass `take.backend_job_id`), `stop_server` (terminate ours, else kill by port via `_pid_on_port`/`_kill_pid`), `server_status`, `monitor_snapshot`, `list_models`, `build_launch_command`, `preflight`, `launch_env`/`clear_gpu_cache`/`gpu_cache_size_mb` (videogen's CUDA kernel cache redirected into an isolated, capped `data/gpu_cache` via `CUDA_CACHE_PATH`/`CUDA_CACHE_MAXSIZE` instead of the global NVIDIA ComputeCache — wipe via the **Clear GPU cache** button or `scripts/clear_gpu_cache.py`; this is the CUDA *compute* cache, NOT the DirectX DXCache), `dynamic_vram_enabled`/`async_offload_enabled` (preflight refuses a server using *either* PCIe weight-streaming path — see rule #10); plus crash-recovery lifecycle: `wait_until_responsive` (poll `server_status` until the server answers), `restart_server` (stop -> `launch_server` -> wait, used after a mid-render crash), and `ensure_server` (launch + wait if the server is *down*, no-op if up — the cold-start path called once before a local render, distinct from the mid-render `restart_server`). During a render `submit()` opens a **best-effort progress WebSocket** (`/ws`) on a daemon thread and feeds per-step fractions to the UI; `progress_fraction()` (pure, headless-testable) maps `progress`/`progress_state` `value`/`max` → a 0..1 fraction. **Robustness bundle (Wave3 A, cards M3/M4/M8/L8/L9/L10, 2026-07-03):** `_api` catches `HTTPError` *separately* from `URLError` and reads the 4xx/5xx body — a `/prompt` validation 400 with `node_errors` (invalid workflow / missing model file) now surfaces its node-level detail (`_http_error_detail`) instead of the misleading "unreachable" message, and a `socket.timeout` read stall / truncated-body `JSONDecodeError` are wrapped as `ComfyError` too (were escaping); `_poll_until_done` tolerates a short run (`_POLL_MAX_CONSECUTIVE_FAILURES`=6) of consecutive `/history` poll failures with a backoff before treating one as fatal, so a transient HTTP stall (ComfyUI's thread blocked loading 14B weights) doesn't lose an hour of GPU work or get misread as a crash (a success resets the streak; the 1h `timeout_s` still bounds it, backoff included; deliberate trade-off: a REAL crash also gets the retries, so crash recovery sees the failure up to ~30s late); `copy_input_image` names the ComfyUI `input/` copy by **content hash** (`kp_<sha1>.<ext>`), not the source name — every local keypose used to be `start.png`/`end.png` deduped by name+byte-size, so two framings that compressed to the same size silently rendered the first one's image (card M8); `restart_server` swallows `subprocess.SubprocessError`/`OSError` (not just `ComfyError`) from the pre-restart `stop_server` so a transient `taskkill` `TimeoutExpired` stall doesn't abandon the whole local queue (card L9); `_claim_output` **raises** on an empty produced list (a `/history` entry with only unrecognized output keys) instead of returning a DONE take pointing at a file that was never written (card L10). Smoke-tested in `smoke_phase2` (`test_comfy_api_errors`, `test_poll_retry`, `test_claim_output_empty`, `test_stop_work_targets_prompt`, `test_restart_server` L9 case, `test_comfy_prepare` M8 content-hash names) |
...
| `backends/crash_recovery.py` | `run_with_crash_recovery` — wraps one local render with crash detection + auto-restart + 3-strike retry (pure / dependency-injected so it's headless-testable): a failure with ComfyUI **down** is a crash (restart the server, retry the same take in place), a failure with it **up** is a genuine workflow error (propagates, fails only that take); on the **final (`MAX_ATTEMPTS`-th) crash it tries one last `restart_server()` + probe before giving up** (card #61) — raises `QueueAbandoned` (caller pauses the local queue) only if that restart fails or the server stays unreachable, else re-raises so just that take fails and the queue keeps running. **Every `QueueAbandoned` MUST go through `_abandon`** (logs the reason, calls `on_abandon`, stamps `CRASH_INTERRUPTED_ATTR`); `run_with_crash_recovery` **asserts `max_attempts >= 1` up front** (Wave3 G / L21) so the loop can't be skipped into the trailing defensive `raise QueueAbandoned` that would bypass that contract. `format_elapsed` (pure) for the "failed in XmYs" note |
| `backends/jobs.py` | `JobManager` on QThreadPool; hosted parallel, local serialized; status signals; `cancel_pending`/`pending_count`; **`cancel_shot_takes(shot_id)`** (cancel just one shot's queued takes) + **`request_stop(take_id)`** (stop an in-flight GENERATING render — comfy `stop_work` / replicate `cancel_prediction`, best-effort; flags the take in `_stopping` so the worker's unwinding backend error records CANCELLED, not FAILED; **`is_stop_requested(take_id)`** lets the hosted runner's `on_submit` self-cancel a prediction whose create-POST returned only after the stop was requested — closing the window where `request_stop` skipped the cancel because `backend_job_id` wasn't recorded yet) + **`abandon_local(reason)`** (pause the local queue after repeated crashes — clears the local pool + cancels still-pending *comfyui* takes, hosted untouched, emits **`queue_abandoned`**) + **`pause_local`/`resume_local`/`is_local_paused`** + **`stop_and_requeue`** (pause/resume the local queue for a batch — rule #16; holds queued local takes as PENDING, keeps their runners in `_runners` for re-enqueue) + **`restart_take`** (re-enqueue a CANCELLED take being restarted in place — clears its stale `_cancelled`/`_stopping`/`_requeue` membership first so the worker doesn't bail, then `enqueue`s; rule #17); `set_project` to switch the active project. **On a project switch with work in flight, the MainWindow drops the batch, then calls `quiesce()` (rule #21)** — it cancels the OUTGOING project's PENDING takes (`interrupted=True`, so they're restartable via rule #17), prunes the stale `_cancelled`/`_stopping`/`_requeue` id sets by intersection (keeping the just-cancelled ids as the dequeue-race safety net and a live worker's stop/requeue flags), but **leaves the currently-GENERATING take running** (its `GenerationJob` captured the old `Project` instance, so its write-through keeps landing in the OLD `takes.json`) and **keeps that take's `_runners` entry**. `live_take_ids()` (= `set(self._runners)`) names the takes a live worker still owns; `has_in_flight_work()` is the pure predicate driving the switch prompt. Worker threads call `project.update_take` (write-through). **On a genuine terminal failure of a HOSTED take the worker fires a best-effort `replicate_client.cancel_prediction` to stop remote spend (follow-up H4):** `GenerationJob._cancel_remote_spend`, called only from the `else` (genuine-failure) branch — NOT the `_stopping`/`_requeue` branches — POSTs a cancel when `backend == "replicate"` AND the take recorded a `backend_job_id` (stamped mid-render by the runner's `on_submit`), because the H4 network retries mean a take that *does* fail terminally (poll retries exhausted / download error / unwind) after the create-POST returned a prediction id otherwise leaves that prediction running to completion at full cost with its output never claimed. It **re-fetches** the take (to read the id `on_submit` persisted, not a stale snapshot), does **not** change the take's terminal status (it stays FAILED — the cancel only stops the bleed, it's not a status change like the deliberate-stop CANCELLED path), skips a never-submitted take (no id) and the comfyui backend, and **swallows all transport errors** like `request_stop` does (a failed cancel must never abort the worker). This is cancel-to-stop-the-bleed, deliberately NOT the reconcile-then-claim (orphan-recovery) alternative that would salvage the output. **On a successful render the worker stamps `fps`/`frame_count` off the produced video via `extract.probe_media_fields` (card #80)** — no runner returns them, so without this a finished take keeps `fps=None` and `export`'s settings.txt prints `fps: None`; the orphan-recovery RECLAIM path (`main_window._execute_plans`) stamps them the same way since it sets `video_path` outside the worker. Emits **`progress_pct(take_id, fraction, label)`** (ephemeral, UI-only — never persisted) alongside the free-text `progress` signal; the runner callback is widened to `progress(line)` for milestones / `progress(frac=.., label=..)` for the step fraction. **Every signal emit inside `GenerationJob.run` MUST go through the guarded `_emit()` helper (card #48)** — a worker thread that emits after its `_JobSignals` C++ object was deleted (project/JobManager churn mid-render) raises `RuntimeError: Signal source has been deleted`, and since `run()` is a QRunnable override invoked from C++ that uncaught exception aborts the whole process (std::terminate, no Python traceback). `_emit` (shiboken6.isValid + try/except RuntimeError) degrades a dead source to a dropped signal; `run()` also wraps its whole body so nothing escapes the override (and still fires `done_cb` so the queue slot is freed). Don't add a raw `self.signals.x.emit()` |
...
| `pipeline/bg_replace.py` | **chroma-key background replacement for assets** (pure / Qt-free, headless-testable — rule #22). Analytic **unmix** ported + generalized from Fighter's `unmixMagenta` (`src/render/assets.ts`): models `observed = a*fg + (1-a)*bg` and, from the per-pixel spill along the screen's colour axis, recovers a **soft alpha + despilled fg** (feathered edges, no coloured halo — far better than `framing._foreground`'s binary corner threshold). `SUPPORTED_CHROMA` (magenta/green/blue + cyan/yellow/red); `unmix_chroma` (generalized by the canonical chroma's hi/lo channel structure — 2-hi `spill=min(hi1,hi2)-lo`, 1-hi `spill=hi-max(lo1,lo2)`; `_FG_MAX`/`_BG_MIN` cutoffs match unmixMagenta); `sample_corner`/`nearest_chroma` (snap a corner sample to the nearest chroma, or None → AUTO); `key_to_transparent` (named chroma → unmix; **AUTO / unknown → falls back to the binary `framing._foreground` keyer** for a non-chroma flat corner); `composite_over` (flatten a transparent sprite onto a solid fill); `replace_background` (key → return both opaque + transparent); `has_transparency`. Smoke: `smoke_phase3.test_bg_replace` |
| `pipeline/extract.py` | frame extraction + thumbnails (PyAV); `video_info` (fps + frame count off a clip) and `probe_media_fields(video_path, fps=None, frame_count=None)` — best-effort: fills whichever of fps/frame_count is `None` by probing the video, leaves them `None` on a missing/unreadable file (never raises). Used to stamp a finished take's `fps`/`frame_count`, which no backend runner reports (card #80) |
...
| `pipeline/takes_io.py` | bin / restore (only files under the project's `.assets/`; external refs left in place). **`move_to_bin` is per-move atomic (M13):** it flips `deleted=True` and writes each successful file move's new path through to the take AS IT HAPPENS (not one batched `update_take` at the end), so a transient AV/indexer lock on a *later* move (the documented Windows failure mode) leaves the record consistent with disk — the already-moved video keeps its `.bin` path and the take is flagged deleted, instead of the video sitting in `.bin` while the record still points at the vanished old path and is never marked deleted. `restore_from_bin` restores such a partial bin cleanly (it moves back only files actually under `.bin`, skipping ones that never moved) **and mirrors the same per-move write-through**, so a failed move-back can't strand an already-restored file's record either — a partial restore is simply retryable. The caller `TakesView.delete` wraps the per-take `move_to_bin` in `try/except OSError` so one take's failed move can't abort binning the rest of a multi-delete |
| `ui/main_window.py` | shot cards + global filters (model + two star filters: **Starred takes** = shots with ≥1 starred take, `starredFilter`; **Starred shots** = shots the user starred, `starredShotsFilter`) + Generate/Export + **Cancel pending** + a **Restart cancelled takes** action (`restart_cancelled_takes`; enabled when the project has ≥1 cancelled take — rule #17) and a right-aligned **Full set** total-price label (`_refresh_total_price`: sums `estimate_cost` over *all* shots, ignoring the view filters; unknown-rate shots tallied as `(+N unknown)`) (the queue actions **Pause batch** / **Cancel pending** / **Restart interrupted takes** are created here via `_build_queue_actions` but render in the **Queue** tab header, not the Shots strip; the Shots-tab control strip keeps the filters, Export view, Generate batch and the Full-set price); **File** menu project lifecycle (**New/Open/Save/Save As** + **New Shot**) with dirty-marker title (`*` when the project is untitled, has buffered edits, or any open shot tab has uncommitted edits — `_has_unsaved_changes`) + save-prompt (and, **before** that save-prompt, an **in-flight-render close warning** — card L5: `closeEvent` calls `_confirm_close_with_inflight_renders()`, which — if any take is GENERATING on **either** backend, per the pure `generating_takes`/`close_warning_text`, the hosted ones flagged as still-**billing** — shows a modal offering *Close anyway* / *Stop renders && close* (best-effort `jobs.request_stop` per take — **stops** spend/GPU, never creates any) / *Cancel* (aborts the close); returns True to proceed. No takes generating → no dialog. The pure decision split is smoke-tested in `smoke_phase2.test_close_warning`); an **Edit** menu whose first entry is **Remove cancelled takes** (`remove_cancelled_takes` → `project.purge_takes` over every cancelled take in the project, ignoring the view filters; confirms first, defaults to No; enabled only when ≥1 cancelled take exists, `_refresh_purge_cancelled_action` refreshed in `reload` **and on every queue status change** — `_on_status_changed`/`_after_job` call it so *Remove cancelled takes* enables live right after Cancel pending, not only on the next reload, L12); a **Settings** menu with a checkable **Update Replicate model data on startup** (persisted via `store/app_settings.py`; when on, `_maybe_refresh_schemas_on_startup` kicks off the Model Library tab's off-thread schema fetch at launch); **closable** tabbed central widget (Shots / Assets / Model Library / ComfyUI Status; reopen from **View**). Double-click a shot row (or **+ New Shot**) opens a shot tab; shot tabs tracked in `shot_tabs`. The open-tab layout (which fixed/shot/take tabs, order, active) is captured into `project.ui_state` on save (`_capture_tab_state`) and rebuilt on open (`_restore_tab_state`) — see the Project model section |
...
| `ui/assets_view.py` | the **Assets** tab: drag-drop / Import keyframe images into `.assets/`; thumbnail grid + delete; a right-click **Replace background…** (rule #22) that keys a source chroma screen out and composites onto a chosen fill (dialog `ui/bg_replace_dialog.py` — source prefilled via `bg_replace.nearest_chroma`, target fill default magenta; `_build_context_menu` is `.exec()`-free for headless tests). Import forces a background composite on a **transparent** asset (`_prepare_imported_asset`: video models can't take alpha), storing the transparent original as the asset's reusable reference. Both flows go through `Project.store_transparent_ref` so a later re-fill reuses the clean transparent sprite. Smoke: `smoke_phase4.test_replace_background_ui` + `test_transparent_import_forces_bg` |
...
| `store/_doc_io.py` | shared JSON-store reader for the three app-global single-file stores below (`schema_cache` / `app_settings` / `prompt_library`). `read_doc(path)` makes the three-way persistence distinction explicit (M11): **absent** (`FileNotFoundError`) → `None` (caller uses seeds/defaults), **readable** → the parsed top-level `dict` (or `{}` if the root isn't an object), **present-but-unreadable/corrupt** (any other `OSError` — notably a transient Windows AV/indexer `PermissionError` — or JSON `ValueError`) → **raises `UnreadableStoreError`**. See rule #20 |
| `store/schema_cache.py` | persistent cache of Replicate input schemas (`data/schema_cache.json`, keyed by `replicate_model_id`); lock-guarded, atomic writes (reuses `store.project._atomic_write_json`). Populated by the Model Library tab; read by the shot editor for per-param enums/types **and to decide whether a model accepts a negative prompt**. Loader routes through `store._doc_io.read_doc` (rule #20): `put` reads `strict=True` (refuses to clobber an unreadable file — raises `UnreadableStoreError`, caught per-model by the Model Library fetch loop), the read-only accessors tolerate it (fall back to `{}`) |
| `store/app_settings.py` | app-global user preferences (`data/app_settings.json`; `get_bool`/`set_bool`). Own file (NOT `app_state.json`, which `main_window._remember_last` rewrites wholesale). Same lock + atomic-write discipline as `schema_cache`, same `store._doc_io.read_doc` loader (rule #20): `set_bool` reads `strict=True` (refuses to clobber an unreadable file — raises `UnreadableStoreError`), `get_bool` tolerates it (falls back to the registered/explicit default). First key `update_schemas_on_startup` (default off) — read by `MainWindow` to auto-refresh Replicate schemas at launch, toggled from the **Settings** menu |
| `store/prompt_library.py` | app-global library of reusable prompt prefabs (`data/prompt_templates.json`; entry = `{name, positive, negative}`, upsert-by-name). Same lock + atomic-write discipline as `schema_cache`; ships seed templates; read/written by the shot tab's Prompt subtab template combo. `_load_list` routes through `store._doc_io.read_doc` (rule #20): `save`/`delete` load `strict=True` (an unreadable file raises `UnreadableStoreError` — must NOT persist 'seeds +/- one entry' and discard every user template), the read-only accessors tolerate it (fall back to seeds) |
...
| `scripts/` | `seed_configs.py` (writes `Fighter.animproj`, imports keyframes as assets) + `smoke_phase*.py` (`smoke_phase1-7` = the always-run gate; **`smoke_phase8.py`** = the opt-in offline mock-ComfyUI integration smoke driving a real take end-to-end via `mock_comfy.make_server`, card #76) + `remote_cli.py` (stdlib client for the `remote/` control server) + `launch_comfyui.py`/`.bat` (local backend, `--disable-dynamic-vram`; both set the isolated CUDA kernel cache via `comfy_client.launch_env()`) + **`clear_gpu_cache.py`** (wipe videogen's isolated CUDA kernel cache in `data/gpu_cache`; `--all` also clears the global pre-isolation NVIDIA ComputeCache — the CUDA compute cache, not the DirectX DXCache) + **`mock_comfy.py`** (the supported **offline GPU-free** fake ComfyUI — drive real local batches with no GPU; see "Offline GPU-free harness" under Run / test / seed). `_`-prefixed scripts are dev/diagnostic scratch (the rule #18 crash hunt: `_app_watch.py`/`_crash_watch.py`/`_widget_census.py`/…) |
| `data/` | runtime (gitignored): `*.animproj` project files (default `Fighter.animproj`) + their sidecar `<name>.assets/` (flat keyframe images + `takes/`, `thumbs/`, `.bin/`); plus `exports/`, `_scratch/` (untitled-project assets), `gpu_cache/` (videogen's isolated CUDA kernel cache, created at ComfyUI launch — see `comfy_client.launch_env`), `app_state.json` (last opened) |
...
- **Deleting a shot is a BUFFERED authoring edit that does NOT destroy its takes early
  ...
  (or Save-As — both commit all buffered authoring edits) drops them for good. **UI-side, `MainWindow.delete_shot` also `_close_take_tabs` for the shot's takes (M10)** — mirroring `remove_cancelled_takes` — so a deleted shot leaves no dangling take-viewer tab (which would otherwise be captured into `ui_state` as a dead descriptor). Previously
...
- `CUDA_CACHE_PATH` / `CUDA_CACHE_MAXSIZE` — **set BY AnimGen** at every ComfyUI launch
  (`comfy_client.launch_env`, mirrored in `launch_comfyui.bat`), not read as config: they
  redirect videogen's CUDA JIT kernel cache into the isolated, gitignored `data/gpu_cache`
  (capped at 2 GiB) instead of the global `%APPDATA%/NVIDIA/ComputeCache`, so it's a single,
  identifiable, bounded, wipeable folder (**Clear GPU cache** button / `scripts/clear_gpu_cache.py`).
  A value already present in the environment is **respected** (`setdefault`), so you can override
  either. This is the CUDA *compute* cache; it has nothing to do with the DirectX **DXCache**
  (filled by desktop/browser/Electron apps — AnimGen's own UI runs software GL per rule #15 and
  writes neither).
...
2. **Additive — copy in, never move external originals.** Importing a keyframe asset
   ...
   neutralize exists. **`move_to_bin` is also per-move atomic (M13):** it flips `deleted`
   and writes each successful file move's new path through immediately (not one batched
   `update_take` at the end), so a mid-sequence AV/indexer lock on a later move leaves the
   record consistent with disk (moved files recorded + `deleted=True`), and `restore_from_bin`
   restores such a partial bin cleanly (it mirrors the same per-move write-through, so a
   partial restore is retryable too); `TakesView.delete` swallows a take's `OSError` so one
   failed move can't abort binning the rest of a multi-delete.
...
4. **Smoke tests run headless** with `QT_QPA_PLATFORM=offscreen`; never call a modal's
   ...
   emit (`shiboken6.isValid` + try/except, card #48/#78). **The shared guard is
   `qt_guard.guarded_emit(obj, signal_name, *args)`** (repo-root module, Wave3 G): route
   EVERY daemon-thread / `QRunnable.run` emit through it — a raw `self.sig.emit(...)` after
   its C++ QObject is deleted raises `RuntimeError('Signal source has been deleted')` and, on
   a thread invoked from C++, aborts the whole process (std::terminate / exit-time SIGSEGV)
   with no traceback. Current sites: `jobs.GenerationJob._emit` and
   `model_library_window._ReplicateRefresher._emit` keep their own method (same body) for the
   hot per-take / per-model paths; `jobs.abandon_local`, `takes_view._StripLoader`,
   `take_player._FrameLoader`/`_GifExporter`, and `main_window._OrphanReconciler` call
   `guarded_emit` directly. Smoke: `smoke_phase5.test_guarded_emit_and_daemon_workers` +
   the two `test_*_survives_deleted_signals`.
...
17. **Cancelled takes can be RESTARTED — exact-snapshot replay in place, else marked failed (card #49).**
    ...
    **Hosted (Replicate) orphan reconciliation on load (card L5, 2026-07-03):** orphan recovery
    used to be **comfy-only** — a HOSTED take a prior session left GENERATING was never reconciled,
    so it kept **billing** on Replicate's servers and sat as a permanent, undismissable "running"
    row in the Queue. `recovery.replicate_orphans(project)` selects those (mirrors `comfy_orphans`:
    replicate-backed, `include_deleted`, generating-before-pending) and the pure
    `recovery.plan_replicate_recovery(orphans, statuses)` reconciles each against its prediction —
    `statuses = {take_id: replicate_client.get_prediction(backend_job_id) or None}`, fetched
    **off-thread** by `main_window._ReplicateReconciler` (an idempotent GET — it **OBSERVES, never
...
18. **The 2026-06-18 native stack-overflow crash is ROOT-CAUSED: a runaway
    ...
    **FIX LANDED (card #72, 2026-06-18):** the earlier "exoneration" of the Queue cell widgets
    ...
    (`_build_context_menu`, no `exec()`) — which also carries a **Stop rendering** entry for a
    GENERATING row (UX1: `_stop` → `jobs.request_stop`, the queue-side mirror of the takes-grid
    entry); the bulk *Cancel pending* button is unchanged. So the
...
20. **App-global store loaders distinguish ABSENT from PRESENT-BUT-UNREADABLE — a mutating op
    must NEVER clobber an unreadable file (M11, card Wave3 C).** The three single-file JSON
    stores (`store/schema_cache.py`, `store/app_settings.py`, `store/prompt_library.py`) share a
    reader, `store._doc_io.read_doc(path)`, that answers the three-way persistence question:
    **absent** (`FileNotFoundError`) → `None` (seeds/defaults are correct); **readable** → the
    ...
    loader (`_load_list`/`_load_doc`) takes a `strict` flag: **mutating ops load `strict=True`**
    (the `UnreadableStoreError` propagates — the write is refused, disk untouched), **read-only
    accessors load `strict=False`** (tolerant — a degraded read falls back to seeds/defaults and
    writes nothing, so it's harmless). Callers: `schema_cache.put`'s only caller (the Model
    Library fetch loop) already wraps per-model in `try/except Exception`, so a refusal is reported
    and the loop continues; `app_settings.set_bool` (Settings-menu toggle lambda) and
    `prompt_library.save` (`shot_tab._save_template`) do **not** yet catch it, so on the rare
    transient lock the raise surfaces as an error rather than as data loss — acceptable and far
    better than the silent clobber (a follow-up could wrap those two GUI slots in a
    retry/toast). Smoke: `smoke_phase2.test_store_absent_vs_unreadable` (monkeypatches
    `builtins.open` to `PermissionError` the store path — the AV-lock analog — and proves
    save/delete/set_bool/put all raise + leave the file intact, while absence still yields seeds).

21. **Switching projects QUIESCES the outgoing queue; it does not abandon its in-flight render
    (M7, card Wave3 H).** `set_project` used to just swap `self.project`, leaving the old
    project's runnables in the pools and stale ids in `_cancelled`/`_stopping`/`_requeue`/`_runners`
    — so project A's takes kept rendering but `pending_count`/`cancel_pending`/`pause_local` saw
    only B (the user couldn't cancel or pause A's queue at all), and an A->B->A switch made
    `_recover_orphans` misclassify the still-rendering take as an orphan (a second monitor of the
    same prompt + two `Project` instances racing last-writer-wins on `takes.json`). The fix,
    entirely in `backends/jobs.py` (`quiesce`/`live_take_ids`/`has_in_flight_work`) +
    `ui/main_window._switch_project`:
    - **Drain-vs-cancel is a user prompt.** `_switch_project` calls
      `_confirm_switch_with_queued_work()` (a QMessageBox split into its own method so the
      decision path is driveable headlessly — tests monkeypatch it, never `.exec()`); it returns
    ...
    - **The currently-GENERATING take is LEFT running.** Its `GenerationJob` captured the OLD
      ...
      after the switch — stopping it would waste the spend/GPU already committed. `quiesce` KEEPS
      that take's `_runners` entry (only the cancelled PENDING ids are popped); its own worker pops
      it via `_on_job_done` when it finishes.
    - **`live_take_ids()` (= `set(self._runners)`) is the orphan-recovery skip-set.**
...
22. **Assets carry an OPT-IN "Replace background" chroma-key, and a transparent import is
    force-composited (video models can't take alpha).** Two distinct keyers exist and serve
    different jobs — don't conflate them: (a) **generation-time** `framing.keyed_sprite` /
    `_foreground` keys a silhouette onto the magenta canvas for *placement* (a crude binary
    corner threshold; unchanged, still robust to any flat fill); (b) **asset-time**
    `pipeline/bg_replace.py` is a much better **analytic unmix** (soft alpha + despill, ported +
    generalized from Fighter's `unmixMagenta`) for cleaning a chroma-screened *source* into a
    flat, opaque, model-valid asset. The Assets-tab **Replace background…** (right-click) runs
    (b): a dialog picks a **source screen to key out** (prefilled by snapping the corner sample
    to the nearest `SUPPORTED_CHROMA`, editable, with an **AUTO** binary-fallback for a
    non-chroma flat corner) **and a separate target fill** (default magenta = the generation
    contract; a non-magenta fill still generates fine since gen re-keys the corner adaptively).
    It overwrites the asset **in place** (same path, so shots keep referencing it). **Import is
    still copy-as-is** (`import_asset` unchanged) EXCEPT a **transparent** asset is
    force-composited onto magenta at import (`assets_view._prepare_imported_asset` — no keying,
    just a flatten) because transparency is invalid model input and gen's
    ...
    snapshot/framing path — rule #3 — and is out of scope). Smoke:
    `smoke_phase3.test_bg_replace` (unmix soft alpha + AUTO fallback + transparency detect),
    `smoke_phase5.test_asset_meta` (ref store/read, survives Save As, remove clears),
    `smoke_phase4.test_replace_background_ui` (prefill + apply + reuse-not-rekey) +
