...
| `ui/shot_card.py` `ui/takes_view.py` | shot row (double-click opens its shot tab; Generate / Export; a **★/☆ star toggle** in the header (`star_btn`/`_refresh_star_btn`, emits `star_toggled`); **right-click context menu** — Edit / Generate / Duplicate / Star (Star/Unstar) / Delete, built in `_build_context_menu` so it's headless-testable without `exec()`) + inline takes folder grid. Each **take** thumbnail carries a **clickable ★/☆ star badge** (top-left corner, painted + hit-tested by `_StarDelegate` on the takes `QListView` via the pure `star_badge_rect`; a click toggles the take's star write-through, mirroring the shot-card star and matching the existing right-click "Toggle star") |
...
- A project is a `Foo.animproj` JSON file (`{format, version, name, shots:[...]}`,
  ...
  (take metadata), `shot_stars.json` (write-through shot stars), the flat keyframe **asset**
  images, and managed media (`takes/`, `thumbs/`, `.bin/`).
